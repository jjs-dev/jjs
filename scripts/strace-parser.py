...
while True:
    ...
    if len(syscall) > 1 and syscall[0].isnumeric(): pid = int(syscall[0])
    else: pid = None
    syscall = syscall[-1]
    args_arr = ['']
    unk_arr = ['']
    ...
    if lxtrace_compat:
        ...
        for i, u in zip(args_arr, unk_arr):
            ...
            if raw.lower().startswith('0x'):
                try: compat_args.append({'kind': 'address', 'data': int(raw, 16)})
                except ValueError: pass
                else: continue
            try: compat_args.append({'kind': 'integral', 'data': int(raw, 8 if raw.isnumeric() and raw.startswith('0') else 0)})
            except ValueError: pass
            else: continue
            u = u.strip()
            if u in ('', '...') and len(i['str_params']) == 1:
                compat_args.append({'kind': 'string', 'data': i['str_params'][0]})
            elif u == '['+', '*(len(i['str_params']) - 1)+']':
                compat_args.append({'kind': 'string_array', 'data': i['str_params']})
            else:
                i2 = dict(i)
                i2['kind'] = 'unknown'
        ...
        if ans != None:
            if ans.startswith('-1 '):
                ...
                compat_ret = {'kind': 'error', 'data': [code, os.strerror(code)]}
            else:
                try:
                    if not ans.lower().startswith('0x'): raise ValueError("not hex")
                    compat_ret = {'kind': 'address', 'data': int(ans, 16)}
                except ValueError:
                    try: compat_ret = {'kind': 'integral', 'data': int(ans, 8 if ans.isnumeric() and ans.startswith('0') else 0)}
                    except ValueError: compat_ret = {'kind': 'unknown', 'raw': ans}
        decoded = {
            'name': syscall,
            'args': compat_args,
            'ret': compat_ret
        ...
        print(json.dumps({'payload': {'kind': 'sysenter', 'data': data}, 'pid': pid}))
    else: print(json.dumps({'syscall': syscall, 'args': args_arr, 'ans': ans}))